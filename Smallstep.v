...
(** **** Exercise: 3 stars, standard, optional (redo_determinism)
...
(** **** Exercise: 3 stars, standard (strong_progress_bool)
...
Proof.
  ...
  - destruct IHt1; inversion H.
    ...
    + (* t1 can step *)
    right. exists (test x t2 t3). apply ST_If. apply H0.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (smallstep_bool_shortcut)
...
(** **** Exercise: 2 stars, standard, optional (test_multistep_4) *)
...
Proof.
  ...
  - (* ST_PlusConstConst *)
    apply E_Plus; eauto; apply E_Const.
  - (* ST_Plus1 *)
    apply E_Plus; auto.
  - (* ST_Plus2 *)
    inversion H0; subst; clear H0; eauto. apply E_Plus; eauto.
Qed.
(** [] *)
...
Proof.
  intros t t' [Hsteps Hnorm]. unfold step_normal_form in Hnorm.
  induction Hsteps.
  - (* multi_refl *)
    rewrite nf_same_as_value in Hnorm.
    destruct Hnorm as [n Hval]. exists n. split; auto.
    apply E_Const.
  - (* multi_step *)
    destruct (IHHsteps Hnorm) as [n' [Heq Heval']].
    exists n'. split; auto.
...
(** **** Exercise: 3 stars, standard, optional (combined_step_deterministic) *)
...
(** **** Exercise: 3 stars, standard, optional (combined_strong_progress) *)
...
(** **** Exercise: 3 stars, standard, optional (compiler_is_correct)
